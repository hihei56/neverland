...
1. `/members consent-link` で「認証する」ボタンを投稿する（案内は任意であることを明記）
   ...
   - `title:` `description:` で Embed の見た目を変えられる（既定は「サーバー名 VERIFY」と定型文）
   - `webhook:True` にすると Webhook 経由で投稿し、送信者名を Embed のタイトル（または `sender_name:`）にできる。Bot に「ウェブフックの管理」権限が必要。投稿後 Webhook は自動で削除する（メッセージとボタンは残る）
...
認証成功のたびに、指定チャンネルへログを投稿します（ユーザー・ID・ロール付与の有無、IP記録が有効なら IP ハッシュ）。ログの投稿に失敗しても、利用者側の完了画面には影響しません。

- `/members authlog channel:#ログ` … 投稿先を設定
- `/members authlog off:True` … 無効化
- `/members authlog` … 現在の設定を表示

`.env` の `AUTH_LOG_CHANNEL_IDS`（`"guildId:channelId,..."` 形式）でも既定値を設定できます。コマンドで設定した内容が優先され、`off` で解除すると既定値も無効になります。
